...
function fetchLastNames() {
  return fetch('https://api.census.gov/data/2010/surname?get=NAME&RANK=1:1000')
    .then(response => response.json())
    .then(names => all_last_names = names)
}

function sortFirstNames() {
  all_first_names.forEach(name =>
    if(name["sex"] === "M") {
      male_first_names.push(name)
    } else if(name["sex"] === "F") {
      female_first_names.push(name)
    }
  )
}
...
function generateName() {
  ...
  last = properly_capitalize(last_names[last_name_index]);
  name = `${first last}`
}
